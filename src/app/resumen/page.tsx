...
export default function ResumenPage() {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/personal-expenses?from=${from}&to=${to}&user_id=${user.id}`);
      if (res.ok) {
              ...
              <button
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    {yearTotal > 0 && <p className="text-xs text-muted">{((month.total / yearTotal) * 100).toFixed(1)}% del año</p>}
                  </div>
